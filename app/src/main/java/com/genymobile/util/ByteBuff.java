...
public class ByteBuff {
    ...
    public void read(InputStream inputStream) throws Exception {
        dev.controller.hecj.cn.controllerdev.util.LogUtil.d("......正在读取数据......");
        int length = inputStream.read(this.array(), this.getWritePostion(), this.getAvaiable());
        dev.controller.hecj.cn.controllerdev.util.LogUtil.d("读位置:"+this.getReadPostion()+",写位置:"+this.getWritePostion()+",Limit长度:"+this.getLimit()+",计划读取:"+this.getAvaiable()+",实际读取长度:"+length);
        if (length == -1) {
            dev.controller.hecj.cn.controllerdev.util.LogUtil.d("......没有读取到数据,休眠10毫秒后继续读取......");
            Thread.sleep(10l);
            return;
        }
    ...
    public void log(){
        dev.controller.hecj.cn.controllerdev.util.LogUtil.d("DataBuffer数据信息,读位置:"+this.getReadPostion()+",写位置:"+this.getWritePostion()+",Limit长度:"+this.getLimit()+",空闲长度:"+this.getAvaiable());
    }
